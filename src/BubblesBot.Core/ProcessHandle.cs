...
public sealed class ProcessHandle : IDisposable
{
    ...
    /// <summary>
    /// True while the originally attached process is still alive. A bot instance cannot safely
    /// follow a restarted PoE client because every memory address belongs to the old process;
    /// callers should exit and attach fresh instead of continuing with stale snapshots/handles.
    /// </summary>
    public bool IsAlive
        => Handle != 0
           && NativeMethods.GetExitCodeProcess(Handle, out var exitCode)
